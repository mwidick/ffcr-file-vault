...
with open(LOG_PATH, "a", encoding="utf-8") as log:
    ...
    try:
        ...
        # --- Patch4b tokens for fallback ---
        OPERATIVE_TOKENS_FALLBACK = [
            "operative","operative_report","operative report","operative-report",
            "op_report","op report","op-report","op. report",
            "or_report","or report","or-report",
            "procedure","procedure report","operation report","operation note",
            "opnote","op note","op-note",
        ]
        def _fallback_hit(link_text, href):
            t = (link_text or "").lower()
            h = (href or "").lower()
            if "/fileattachment/" not in h:
                return False
            return any(tok in h or tok in t for tok in OPERATIVE_TOKENS_FALLBACK)
        # --- Patch4b additions ---
        OPERATIVE_TOKENS_FALLBACK = [
            "operative","operative_report","operative report","operative-report",
            "op_report","op report","op-report","op. report",
        ...
        def _fallback_hit(link_text: str, href: str) -> bool:
            t = (link_text or "").lower()
            h = (href or "").lower()
            if "/fileattachment/" not in h:
                return False
        ...
        def robust_open_patient(mrn, max_tries=3):
            ...
            while tries < max_tries:
                ...
                try:
                    ...
                    log_msg(log, f"[RETRY-OPEN:{tries}] stale element → reloading patient list")
                    try:
                        driver.get("https://entaaf.ema.md/ema/web/practice/staff#/practice/staff/patient/list")
                        time.sleep(1.2)
                    except Exception:
        ...
        def force_download_dir(dir_path: str):
            """
            Best-effort: set Chrome download directory for this session using CDP.
            No-op if CDP method is unavailable; fallback still works via requests.Session.
            """
            try:
                import json as _json
                driver.execute_cdp_cmd("Page.setDownloadBehavior", {
                    "behavior": "allow",
                    "downloadPath": os.path.abspath(dir_path)
                })
                log_msg(log, f"[DL PATH] Set Chrome download dir to: {os.path.abspath(dir_path)}")
            except Exception as e:
                log_msg(log, f"[DL WARN] setDownloadBehavior failed: {e}")
        ...
        def extract_operatives_all_pages(mrn):
            ...
            # Open Attachments and set per-page to 100
            open_attachments_tab()
            try:
                _ = set_per_page_to_100()
            except Exception as e:
                log_msg(log, f"[WARN] set_per_page_to_100 ultimately failed: {e} (continuing at default page size)")
            ...
            # ===== FFCR DBG3 (Patch4b): Hard-click fallback if normal pass saved 0 =====
            try:
                if saved == 0:
                    ...
                    for a in hits:
                        try:
                            ...
                            if not href:
                                continue
                            fname = href.split("/")[-1].split("?")[0] or f"operative_{int(time.time()*1000)}.pdf"
                            r = session.get(href, timeout=90)
                            if r.ok and r.content:
                                final = safe_join(mrn_dir, fname)
                                with open(final, "wb") as f:
                                    f.write(r.content)
                                log_msg(log, f"[FFCR-DBG3] OP-SAVED via hardget: {fname}")
                                downloaded += 1
                            else:
                                log_msg(log, f"[FFCR-DBG3] hardget failed ({r.status_code}) for {fname}")
                        except Exception as e:
                            log_msg(log, f"[FFCR-DBG3] hardget error: {e}")
                    saved += downloaded
            except Exception as e:
